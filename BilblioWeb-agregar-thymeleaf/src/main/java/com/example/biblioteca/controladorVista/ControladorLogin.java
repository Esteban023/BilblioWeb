...
public class ControladorLogin {
    ...
    public String login(RedirectAttributes redirect, HttpSession session, @RequestParam String codigo) {
        ...
        if(codigo == null || codigo.trim().isEmpty()){
            redirect.addFlashAttribute("error", "El codigo no puede estar vacio");
            return "redirect:/login";
        }

        if (codigo.length() < 8 || codigo.length() > 10) {
            redirect.addFlashAttribute("error", "El codigo debe tener entre 8 y 10 digitos");
            return "redirect:/login";
        }
